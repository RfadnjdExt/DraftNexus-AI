...
def preprocess_features(df_train, df_stats):
    ...
    for _, row in df_train.iterrows():
        ...
        # 2. Ally Role Counts (New Explicit Feature)
        # 1:Exp, 2:Mid, 3:Roam, 4:Jungle, 5:Gold
        role_counts = [0, 0, 0, 0, 0] # Index 0->Lane1, 1->Lane2...
        for h in ally_ids:
            # Look up primary lane from stats
            # We need stats_map to cover ALL heroes, not just candidate.
            # Currently stats_map uses 'to_dict', which is efficient.
            # But we filtered stats_map to only Candidate columns? 
            # actually we used df_stats... let's check scope.
            pass
            
        # Fix: We need a quick lookup for Primary Lane for ALL heroes
        # We can build a simple dict outside the loop
...
def train_model():
    ...
    stat_imps = importances[-7:] # Last 7
